...
plt.savefig("results/concentration_profile.png", dpi=200)
plt.close()
...
plt.tight_layout()
plt.savefig("results/flux_vs_time.png", dpi=200)
plt.close()

# raw data (time + flux) to CSV
df = pd.DataFrame({
    "time (s)": times,
    "flux (mol/m^2/s)": fluxL
})
df.to_csv("results/raw_data.csv", index=False)
print(" Saved: results/concentration_profile.png, results/flux_vs_time.png, results/raw_data.csv")
